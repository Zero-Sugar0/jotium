  ...
  return (
    <div className="flex min-h-screen w-screen items-center justify-center bg-background pt-16 pb-8">
      <div className="w-full max-w-md overflow-hidden rounded-2xl flex flex-col gap-12">
        ...
        {/* OAuth Buttons */}
        <div className="flex flex-row gap-1.5 px-4 sm:px-16">
          <button
            onClick={() => handleOAuthSignIn("google")}
            disabled={isLoading || state.status === "in_progress"}
            className="flex-1 flex items-center justify-center gap-1.5 bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-600 rounded-lg px-2 py-1.5 text-sm font-medium text-gray-700 dark:text-zinc-200 hover:bg-gray-50 dark:hover:bg-zinc-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
          >
            {isLoading ? (
              <div className="size-3.5 border-2 border-gray-300 border-t-gray-600 rounded-full animate-spin" />
            ) : (
          ...
          <button
            ...
            disabled={isLoading || state.status === "in_progress"}
            className="flex-1 flex items-center justify-center gap-1.5 bg-black dark:bg-white text-white dark:text-black rounded-lg px-2 py-1.5 text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
          >
            {isLoading ? (
              <div className="size-3.5 border-2 border-gray-300 border-t-gray-600 rounded-full animate-spin" />
            ) : (

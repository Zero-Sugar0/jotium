...
export class N8NTool {
  ...
  getDefinition(): FunctionDeclaration {
    return {
      name: "n8n_automation",
      description: "Advanced comprehensive enterprise-grade workflow automation and integration platform using n8n for complete business process automation, system integration, and intelligent workflow orchestration. Create sophisticated multi-step workflows with visual drag-and-drop interface connecting 400+ services including AI services (OpenAI, Anthropic, Google AI), databases (PostgreSQL, MySQL, MongoDB), cloud platforms (AWS, Azure, GCP), communication tools (Slack, Teams, Discord), CRM systems (Salesforce, HubSpot, Pipedrive), e-commerce platforms (Shopify, WooCommerce, Magento), social media (Twitter, LinkedIn, Facebook), file storage (Google Drive, Dropbox, S3), email services (Gmail, Outlook, SendGrid), and custom APIs. Manage secure credential storage with encrypted authentication tokens, API keys, and connection strings. Monitor real-time workflow executions with detailed logging, error handling, retry mechanisms, and performance metrics. Build complex conditional logic with branching workflows, loops, data transformation, and dynamic routing. Implement scheduled automation with cron expressions, interval triggers, and event-based activation. Create reusable workflow templates for common business processes like data synchronization, notification systems, approval workflows, and reporting automation. Handle webhook endpoints for real-time data processing and external system integration. Execute workflows synchronously or asynchronously with queue management and load balancing. Perfect for enterprises, developers, system integrators, and businesses requiring sophisticated workflow automation, business process management, system integration, and intelligent process orchestration across multiple platforms and services.",
      parameters: {
        type: Type.OBJECT,
        properties: {
          action: {

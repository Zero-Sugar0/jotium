...
export class LinearManagementTool {
  ...
  getDefinition(): FunctionDeclaration {
    return {
      ...
      description: "Complete Linear project management and team collaboration platform for comprehensive issue tracking, project planning, and workflow automation. Create, update, and manage issues with detailed tracking including priority levels, estimates, due dates, assignees, labels, and workflow states. Organize projects with milestones, team assignments, and progress tracking. Manage teams with member roles, permissions, and team-specific workflows. Plan and track development cycles with sprint management, velocity tracking, and burndown analytics. Handle user management with profile settings, activity tracking, and team memberships. Add comments with markdown support, reactions, and threaded discussions. Attach files, documents, and external resources to issues. Create and manage labels for categorization and filtering. Configure workflow states for custom development processes. Access organization settings, member management, and integration configurations. Generate comprehensive analytics including issue velocity, team performance, project progress, and cycle completion rates. Export data in multiple formats (JSON, CSV, Markdown, PDF) with customizable filtering. Set up webhooks for real-time notifications and integrations. Perform batch operations for bulk updates and data synchronization. Execute custom GraphQL queries and mutations for advanced use cases. Perfect for development teams, project managers, product teams, and organizations using Linear for comprehensive project management, agile development, issue tracking, and team collaboration.",
      parameters: {
        type: Type.OBJECT,
        properties: {
          operation: {

...
export class SupabaseTool implements Tool {
  ...
  getDefinition(): FunctionDeclaration {
    return {
      ...
      description: "Advanced full-stack database management and backend-as-a-service platform using Supabase for complete application development with PostgreSQL, real-time subscriptions, authentication, file storage, and edge functions. Perform comprehensive database operations including CRUD operations (select, insert, update, delete, upsert), advanced queries with joins and aggregations, raw SQL execution, stored procedures, and transaction management. Manage user authentication with email/password, OAuth providers (Google, GitHub, etc.), password reset, email verification, session management, and user profile updates. Handle file storage with bucket management, file upload/download, signed URLs, and storage policies. Implement real-time features with subscriptions, broadcasting, presence tracking, and live data synchronization. Deploy and manage edge functions for serverless computing. Create and manage database schemas, tables, indexes, views, and schemas with full DDL support. Implement security with Row Level Security (RLS) policies, role-based access control, and authentication rules. Perform backup and migration operations, data export/import, and table backups. Monitor database performance with query analytics, connection statistics, and table statistics. Access project management APIs and health monitoring. Perfect for developers building modern web applications, mobile apps, SaaS platforms, and enterprise solutions that require a scalable backend with real-time capabilities, user management, and comprehensive database functionality.",
      parameters: {
        type: Type.OBJECT,
        properties: {
          action: {
